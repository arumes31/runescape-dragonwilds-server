...
export class Controller {
  constructor({ docker = dockerClient(), container, project, hour = null, stateFile = null, autoUpdate = true, versionProvider = querySteam, installedProvider = installedBuild }) {
  ...
  save() {
    ...
    writeFileSync(this.stateFile + '.tmp', JSON.stringify({ lastDate: this.lastDate, lastAction: this.lastAction, lastAutoRestartAt: this.lastAutoRestartAt }), { mode: 0o600 });
  ...
  async status() {
    ...
    return { name: this.container, state: data.State.Status, running: data.State.Running,
  ...
  action(action, source = 'admin') {
    ...
    this.perform(action).catch(error => {
  ...
  async automaticRestart(source, now = new Date()) {
    const allowed = () => !this.busy && (!this.lastAutoRestartAt || now.getTime() - Date.parse(this.lastAutoRestartAt) >= 7200000);
    if (!allowed()) return;
    const data = await this.inspect();
    ...
    this.lastAutoRestartAt = now.toISOString();
    this.save(); // Fail closed if the cooldown cannot be persisted.
    this.action('restart', source);
  }
  async checkUpdates(now = new Date()) {
    ...
    try {
      ...
      this.versions = { installedBuild: installed, latestBuild: latest, checkedAt: now.toISOString(), error: null, updateAvailable: available };
      if (available && this.autoUpdate) await this.automaticRestart('steam-update', now);
    } catch (error) {
      this.versions = { ...this.versions, checkedAt: now.toISOString(), error: error.message, updateAvailable: false };
    } finally { this.checkingVersions = false; }
  }
  async maintain(now = new Date()) {
    if (this.hour === null || now.getUTCHours() !== this.hour || this.busy) return;
    ...
    try {
      ...
      if (!data.State.Running) {
        this.lastAction = { at: now.toISOString(), source: 'schedule', ok: true, message: 'Skipped: server is stopped' };
      ...
      this.lastAction = { at: now.toISOString(), source: 'schedule', ok: false, message: error.message };
    }
    this.save();
  }
}
...
export function createApp({ password, controller, origins, secureCookies = false, sessionMs = 8 * 60 * 60 * 1000 }) {
  ...
  return http.createServer({ requestTimeout: 10000, headersTimeout: 10000 }, async (request, response) => {
    ...
    try {
      ...
      if (path === '/api/login' && request.method === 'POST') {
        ...
        if (attempt.count >= 10 || attempts.size > 1000) return reply(response, 429, { error: 'Too many attempts. Try again in 15 minutes.' });
        ...
        if (typeof input.password !== 'string' || !timingSafeEqual(scryptSync(input.password, salt, 64), hash)) return reply(response, 401, { error: 'Incorrect admin password' });
        attempts.delete(ip);
      ...
      reply(response, error instanceof SyntaxError ? 400 : 502, { error: error instanceof SyntaxError ? 'Invalid JSON' : 'Request failed. Check the admin service logs and Docker connection.' });
    }
  });
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const hour = process.env.AUTO_UPDATE_UTC_HOUR ?? 'off';
  const controller = new Controller({ container: process.env.GAME_CONTAINER ?? 'dragonwilds-game', project: process.env.COMPOSE_PROJECT_NAME ?? 'dragonwilds', hour: hour === 'off' ? null : Number(hour), autoUpdate: process.env.AUTO_UPDATE !== 'false', stateFile: process.env.STATE_FILE ?? '/data/maintenance.json' });
  ...
  const app = createApp({ password, controller, origins, secureCookies: origins.every(origin => origin.startsWith('https://')) });
